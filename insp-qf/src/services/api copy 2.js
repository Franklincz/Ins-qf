...
async getReportes({ status = 'all', limit = 20, cursor = null } = {}) {
    ...
    // 🔹 Normaliza: no mandar "all", solo mandar si hay filtro real
    const statusMap = {
      approved: 'aprobado',
      pending: 'pendiente',
      rejected: 'rechazado',
    ...
    try {
      ...
      throw error; // ❌ ya no hay fallback a JSON hardcode
    }
  }
  ...
  // ----------------------------------------------------
  // Upload URL (Storage firmado desde backend)
  ...
  /**
   * Obtiene el resumen/series para el dashboard desde el backend.
   * @param {{rangeDays?: number}} opts
   * @returns {Promise<{
   *   summary: { total:number, approved:number, pending:number, rejected:number, withPdf:number, approvalRate:number, rejectRate:number, avgDefects:number, avgCycleDays:number|null },
   *   breakdown: any,
   *   series: any
   * }>}
   */
