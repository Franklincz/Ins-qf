...
export default function Sidebar({
  ...
  isOpen,
  setIsOpen,
  /** si true, agrega el Spacer (por defecto lo desactivamos porque el Layout ya empuja el contenido) */
  useSpacer = false,
}) {
  ...
  // persistencia SOLO cuando es no-controlado
  useEffect(() => {
    if (setIsOpen) return; // está controlado desde fuera
  ...
  return (
    <>
      ...
      {/* Drawer móvil */}
      <MobileAside className="sidebar-transition" data-open={mobileOpen} aria-label="Menú">
        <Header>
          <BrandRow>
        ...
        <Footer>
          <SidebarItem icon={LogOut} text="Cerrar sesión" expanded onClick={handleLogout} size="lg" />
          {userInfo && (
            <UserCard>
              <Avatar>{(userInfo.name || "U").split(" ").map(n=>n[0]).join("").slice(0,2).toUpperCase()}</Avatar>
              <UserMeta>
      ...
      <DesktopAside className="sidebar-transition" data-expanded={expanded} aria-label="Menú">
        ...
        <Footer>
          <SidebarItem icon={LogOut} text="Cerrar sesión" expanded={expanded} onClick={handleLogout} size="lg" />
          {userInfo && (
      ...
      {/* Empuje de contenido: SOLO si lo piden (por compatibilidad) */}
      {useSpacer && (
